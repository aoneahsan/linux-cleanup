...
|---|---|
| Package-manager caches | File by file: `~/.npm/_cacache`, `~/.yarn/berry/cache`, `~/.local/share/pnpm/store`, `~/.cache/pip`, `~/.cache/composer`. **Whole entries only** (one package / one npx tree at a time): `~/.cache/yarn`, `~/.npm/_npx`, `~/.bun/install/cache` |
| App caches | Chrome / Brave / Chromium / Edge / Vivaldi `Cache/`, Firefox `cache2/`, Cypress / Playwright builds and TypeScript typings (whole versions only), `~/.zoom/Cache`, VSCode / Cursor `CachedExtensionVSIXs` |
